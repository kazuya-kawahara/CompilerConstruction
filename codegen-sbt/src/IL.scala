  ...
  def trans(e: Abssyn.Exp) : (Code, Val) = 
    e match {
      ...
      case Abssyn.UOpExp(o, e1) => {
        val x = freshVar()
        val (c1, v1) = trans(e1)
        (c1 ++ List(AssignInstr(x, UOpExp(o, v1))),
          VarVal(x))
      }
      case Abssyn.IfExp(e, e1, e2) => {
        ...
        val (c1, v1) = trans(e1)
        val (c2, v2) = trans(e2)
        (List(IfGotoInstr(ce, thenLabel)) ++ c2 ++ List(AssignInstr(r, ValExp(v2))) ++
        List(GotoInstr(nextLabel)) ++ List(LabelInstr(thenLabel)) ++
        c1 ++ List(AssignInstr(r, ValExp(v1))) ++ List(LabelInstr(nextLabel)),
          VarVal(r))
      }
      case Abssyn.AppExp(f, es) => {
        val r = freshVar()
        val (cs, vs) = es.map(trans(_)).unzip
        (cs ++ List(AssignInstr(r, AppExp(f, vs.map(ValExp(_))))),
          VarVal(r))
      }
    }
